    ...
    return {
      ...
      getRepoHost: (): string => {
        ...
        throw new ExitFailedError(
          "Could not determine the host of this repo (e.g. 'github.com' in the repo 'https://github.com/danerwilliams/gs'). Please run `gt repo owner --set <owner>` to manually set the repo owner."
        );
      },
      ...
      getRepoOwner: (): string => {
        ...
        throw new ExitFailedError(
          "Could not determine the owner of this repo (e.g. 'gs' in the repo 'danerwilliams/gs'). Please run `gt repo owner --set <owner>` to manually set the repo owner."
        );
      },
      ...
      getRepoName: (): string => {
        ...
        throw new ExitFailedError(
          "Could not determine the name of this repo (e.g. 'gs' in the repo 'danerwilliams/gs'). Please run `gt repo name --set <owner>` to manually set the repo name."
        );
      },
    } as const;
  },
...
function inferRepoGitHubInfo(remote: string): {
  ...
  const inferError = new ExitFailedError(
    `Failed to infer the owner and name of this repo from remote ${remote} "${url}". Please run \`gt repo owner --set <owner>\` and \`gt repo name --set <name>\` to manually set the repo owner/name. (e.g. in the repo 'danerwilliams/gs', 'gs' is the repo owner and 'gs' is the repo name)`
  );
  if (!url) {
    throw inferError;
  }
