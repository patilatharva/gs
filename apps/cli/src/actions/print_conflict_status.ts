...
export function printConflictStatus(
  ...
  context.splog.info(
    chalk.yellow(`To fix and continue your previous gs command:`)
  );
  context.splog.info(`(1) resolve the listed merge conflicts`);
  context.splog.info(
    `(2) mark them as resolved with ${chalk.cyan(`gs add .`)}`
  );
  context.splog.info(
    `(3) run ${chalk.cyan(
      `gs continue`
    )} to continue executing your previous gs command`
  );
  context.splog.info(
    "It's safe to cancel the ongoing rebase with `gs rebase --abort`."
  );
}
